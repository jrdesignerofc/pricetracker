...
import { parsePriceFromAnyScript } from "./utils";

export async function scrapeTerabytePrice(url: string): Promise<{ price: number; currency?: string }> {
  ...
  const fromJson = parsePriceFromAnyScript($);
  ...
  throw new Error("price-not-found-terabyte");
}

export default scrapeTerabytePrice;
